-- Fix organizational catalog schema drift.
-- Some environments still have departments/provinces/districts as geographic
-- catalogs, while the API now expects internal departments and geographic_*.
...
BEGIN
  IF to_regclass('public.departments') IS NOT NULL
    ...
    IF to_regclass('public.geographic_departments') IS NULL THEN
      ALTER TABLE public.departments RENAME TO geographic_departments;
    ELSE
      ALTER TABLE public.departments RENAME TO departments_legacy_geographic_backup;
    END IF;
  END IF;

  IF to_regclass('public.provinces') IS NOT NULL THEN
    IF to_regclass('public.geographic_provinces') IS NULL THEN
      ALTER TABLE public.provinces RENAME TO geographic_provinces;
    ELSE
      ALTER TABLE public.provinces RENAME TO provinces_legacy_geographic_backup;
    END IF;
  END IF;

  IF to_regclass('public.districts') IS NOT NULL THEN
    IF to_regclass('public.geographic_districts') IS NULL THEN
      ALTER TABLE public.districts RENAME TO geographic_districts;
    ELSE
      ALTER TABLE public.districts RENAME TO districts_legacy_geographic_backup;
    END IF;
  END IF;
END $$;
  ...
  FOR constraint_name IN (
    ...
    WHERE conrelid = 'public.areas'::regclass
      ...
      AND conkey = ARRAY[
        (SELECT attnum
         FROM pg_attribute
         WHERE attrelid = 'public.areas'::regclass
...
WHERE a.company_id IS NOT NULL
  AND NOT EXISTS (
    ...
    WHERE d.company_id = a.company_id
      AND LOWER(d.name) = LOWER('Administracion')
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_areas_department_id'
  ) THEN
    ALTER TABLE public.areas
...
CREATE INDEX IF NOT EXISTS idx_areas_company_department_active
  ON public.areas(company_id, department_id, is_active) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_job_positions_company_area_active
  ON public.job_positions(company_id, area_id, is_active) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_workers_labor_assignment
  ON public.workers(company_id, internal_department_id, area_id, position_id, work_location_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_geographic_departments_name
  ON public.geographic_departments(LOWER(name)) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_geographic_provinces_name
  ON public.geographic_provinces(department_id, LOWER(name)) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_geographic_districts_name
  ON public.geographic_districts(province_id, LOWER(name)) WHERE deleted_at IS NULL;
